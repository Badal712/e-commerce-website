...
<%!
	...
	String invN, invD, reqN, reqD, supN, supID, loc, ct, phNo, mid, gstN, invT, slNo, proID, proN, cat, pri, purQ, amo, disP, disA, gstP, gstA, nAmo;
	...
	<head>
		<%
			...
			try{
				...
				stmt.executeUpdate("insert into osp_tblrecgood value('"+invN+"', '"+invD+"', '"+reqN+"', '"+reqD+"', '"+supN+"', '"+supID+"', '"+loc+"', '"+ct+"', '"+phNo+"', '"+mid+"', '"+gstN+"', '"+invT+"','"+slNo+"', '"+proID+"', '"+proN+"', '"+cat+"', '"+pri+"', '"+purQ+"', '"+amo+"', '"+disP+"', '"+disA+"', '"+gstP+"', '"+gstA+"', '"+nAmo+"')");
				
				//Update stock data in the product table in the database.
				
				stmtStock = conn.createStatement();
				stmtStock.executeUpdate("update osp_tblproduct set Stock = Stock + '"+purQ+"' where Product_Code = '"+proID+"'");
				
				//Code for show alert
					
				out.println("<script type='text/javaScript'>");
				out.println("alert("+"'Your data Saved Successfully...'"+");\n");
				out.println("setTimeout(function(){window.location.href='OSP_RecGoodc.jsp'},10);");
				out.println("</script>\n");
			}
			catch(SQLException se){
